...
from bot.services import detector as detector_module
...
def _process_tasks(
    ...
    for task_num, task in enumerate(tasks, start=1):
        ...
        # If the type wasn't determined at parse time (by design — see
        # file_handlers.py), detect it now, before deciding the download
        # strategy (yt-dlp vs requests) or the upload strategy (send_video
        # vs send_document). Without this, url_type stays "unknown" for
        # the entire task and video uploads incorrectly fall back to
        # send_document.
        if url_type == "unknown":
            url_type = detector_module.detect_url_type(url)
            task_repo.update_url_type(task_id, url_type)
